...
```sql
menu_items (id, name, category, style, base_price, has_bean_choice, active)
  -- category: signature / coffee / matcha / non_coffee / special
  -- style (เฉพาะ category=coffee): milk / orange / black — ใช้จัดกลุ่มปุ่มใน Staff Panel เท่านั้น
  -- ตัวอย่าง: Latte / coffee / milk / 60 / true
  --          Drip Coffee / coffee / black / 100 / true   ← ดริปเป็นเมนูของตัวเอง ไม่ใช่ add-on
  --          Matcha Latte / matcha / null / 85 / false
...
ข้อมูลเมนูทั้งหมด (Signature, Coffee, Matcha, Non-Coffee) ใส่ไว้ครบแล้วตาม migration `20260705040000_menu_items.sql` + `20260705050000_menu_item_style.sql`

**หลักการสำคัญที่ตกลงกันไว้**: เก็บข้อมูลระดับเมนูจริงเสมอ (ไม่ยุบรวม Latte/Cappuccino เป็นก้อนเดียวในฐานข้อมูล) — การจัดกลุ่มเพื่อความเร็วทำที่ชั้น **UI ของ Staff Panel เท่านั้น** (กด "กาแฟ" → กด "นม/ส้ม/ดำ" ตามคอลัมน์ `style` → กด "เมล็ด" → ค่อยเจาะจงเมนู เช่น Latte vs Cappuccino) ป้องกันไม่ให้เสียข้อมูลที่จำเป็นสำหรับคำนวณ badge ในอนาคต ส่วนหน้าตา icon/favicon ค่อยออกแบบทีหลัง (Phase 2 UI)
