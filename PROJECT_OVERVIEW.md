...
└── supabase/
    ...
    └── migrations/
        └── (ดูรายการ migration ทั้งหมดในโฟลเดอร์ — ล่าสุด 20260706030000)
```
...
- [ ] **ยังไม่ได้ทดสอบ `create-order-token`/`claim-order-token` แบบ end-to-end จริง** — ต้องทดสอบต่อ (ดูวิธีทดสอบด้านล่าง)
- [ ] **เขียนหน้า Staff Panel จริง** (ตอนนี้มีแค่ Edge Function backend ยังไม่มีหน้าเว็บ UI ให้พนักงานกด — ต้องมีก่อนถึงจะเรียก `create-order-token` ได้จากหน้าจอจริง)
- [ ] เขียนหน้า Admin Dashboard และ Buddy Book เวอร์ชันเต็มตาม wireframe (ตอนนี้ของจริงบน GitHub Pages ยังเป็นเวอร์ชันทดสอบเปล่าๆ — โชว์แค่ชื่อ+แต้ม)
- [ ] เขียน Reward Engine logic ส่วนที่เหลือ (คำนวณ tier, ตรวจ badge, ออก coupon วันเกิด)
- [ ] ทำ staff-login (Supabase email/password, Option A) แล้วเอามาป้องกัน `create-order-token`

**วิธีทดสอบ `create-order-token`/`claim-order-token` วันนี้แบบไม่ต้องรอ Staff Panel**: เรียก `create-order-token` ตรงๆ ผ่านหน้า Supabase Dashboard (Edge Functions → เลือกฟังก์ชัน → ปุ่ม Test/Invoke) ใส่ `menu_item_id` จริงจากตาราง `menu_items` → เอา `token` ที่ได้มาแปะเป็น URL param ทดสอบกับหน้า Buddy Book ที่ deploy อยู่แล้ว (ต้องเพิ่มปุ่ม/logic เรียก `claim-order-token` ในหน้านั้นก่อนถึงจะทดสอบจบ end-to-end ได้ — ยังไม่ได้ทำ)
