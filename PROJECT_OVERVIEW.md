...
- [ ] **`members.tier` ไม่อัปเดตอัตโนมัติตามแต้มสะสม** — gap ที่เพิ่งเจอ ตอนนี้ตั้งเป็น `'sip'` ตอนสมัครแล้วไม่มี logic ไปคำนวณใหม่เลยแม้แต้มจะเพิ่มขึ้นเรื่อยๆ ต้องเขียน logic เช็คช่วงแต้ม (Sip 0-99 / Drink 100-299 / Slurpp 300+) แล้วอัปเดตทุกครั้งที่แต้มเปลี่ยน (ใส่ใน `claim_order_token()` function หรือ trigger บนตาราง `members`) — **ควรทำเป็นลำดับถัดไป**
- [ ] เขียนหน้า Admin Dashboard เวอร์ชันเต็มตาม wireframe (ยังไม่มีโค้ดจริง มีแค่ wireframe)
- [ ] ตกแต่ง Staff Panel/Buddy Book ให้ตรง CI (ตอนนี้ยังเป็น UI เปล่าๆ เน้นให้ทำงานถูกต้องก่อน) — Buddy Book ยังโชว์แค่ชื่อ+แต้ม ยังไม่มี Tier badge/progress bar/Badge/คูปอง/ประวัติ/หน้าโปรไฟล์ตาม wireframe
- [ ] เขียน Reward Engine logic ส่วนที่เหลือ (ตรวจ badge unlock, ออก coupon วันเกิดอัตโนมัติ)
- [ ] ทำ staff-login (Supabase email/password, Option A) แล้วเอามาป้องกัน `create-order-token` (ตอนนี้ใครก็เรียกได้ถ้ารู้ URL — ความเสี่ยงต่ำตอนนี้เพราะยังไม่เปิดใช้จริง แต่ต้องปิดก่อน launch)
...
**URL หน้าจริง**: Staff Panel = `https://webwecreate.github.io/buddy-brew-plaform/staff.html`, Buddy Book = `https://webwecreate.github.io/buddy-brew-plaform/` (เปิดตรงๆ ในเบราว์เซอร์ปกติได้เพราะเป็นเว็บธรรมดา ไม่ใช่ LIFF)
