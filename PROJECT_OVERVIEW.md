...
- [x] **Buddy Book (`docs/index.html`) v1 — ทำ bottom tab bar (สมาชิก/โปรโมชั่น/ติดต่อ) + tier progress bar + หน้า "Tier·แต้ม·รางวัล" + ประวัติการได้แต้ม (last 3 + ดูทั้งหมด) แล้ว** — เพิ่ม Edge Function ใหม่ `get-member-history` (verify LINE idToken แบบเดียวกับ `create-or-get-member`, คืนประวัติ `points_transactions` ของสมาชิกคนนั้น) + migration `20260707040000_promotions_anon_read.sql` เปิดให้ anon อ่าน `promotions` ได้ (grant+policy คู่กันตามแบบ `menu_items`) — flow เคลม QR เดิม (`?token=...` → celebration 3 วิ → เข้าโปรไฟล์) ยังทำงานเหมือนเดิมทุกอย่าง แค่ปลายทางเปลี่ยนเป็นหน้าที่มี tab แล้ว — แท็บติดต่อยังเป็น placeholder รอข้อมูลจริงจากร้าน (เวลาเปิด-ปิด/ที่อยู่/เบอร์โทร/ลิงก์ LINE OA)
- [x] **Badge section ทำแล้ว (Reward Engine ส่วน badge unlock เสร็จ)** — seed badge 6 อัน ที่คำนวณได้จากข้อมูลจริง (Espresso Lover, Latte Master, Matcha Fan, Early Bird, Night Owl, Buddy Friend — ไม่รวม Rain Hunter เพราะต้องมี weather API หรือปุ่มให้ staff ติ๊กเอง) migration `20260707060000_badge_seed_and_unlock.sql` เพิ่ม trigger `check_badge_unlocks()` (AFTER INSERT ON points_transactions) ตรวจ 5 badge ที่ผูกกับ purchase/เวลาอัตโนมัติทุกครั้งที่มีแต้มเข้า — ส่วน Buddy Friend (ครบ 1 ปี) เป็นแบบผูกกับวันที่ไม่ใช่ event เลยเช็คแบบ lazy ใน `get-member-history` แทน (ทุกครั้งที่เปิด Buddy Book) — `get-member-history` คืนทั้ง `history` และ `badges` (พร้อม earned/locked state) ในคำตอบเดียวกัน — Buddy Book โชว์ 3 badge ที่ปลดแล้วในหน้าหลัก + "ดูทั้งหมด" เปิดหน้าเห็นทั้ง 6 badge (ปลดแล้ว/ยังไม่ปลด) — **ยังไม่ทำ**: คูปอง (ยังไม่มีระบบออกคูปอง), ฟิลด์วันเกิด+โบนัส 20 แต้ม (ต้องมี edge function ใหม่ให้ลูกค้าแก้ข้อมูลตัวเองได้ก่อน ตอนนี้ลูกค้าไม่มีสิทธิ์เขียน `members` เลย), Rain Hunter badge (รอตัดสินใจ weather API vs. staff ติ๊กเอง)
